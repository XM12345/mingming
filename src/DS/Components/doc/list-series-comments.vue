...
export default Vue.extend({
  name: 'doc--list-series-comments',
  props: {
    seriesId: { required: true, type: Number, default: 0 }
  },
  data() {
    return {
      comments: [] as any[]
  ...
  methods: {
    async init() {
      this.comments = await this.$Model.Doc.series_comments(this.seriesId);
    }
  }
});
</script>
