<template>
  <div :class="[B()]">
    ...
    <div v-show="item.content" v-for="item in comments" :key="item.id" :class="[B('__item')]">
      ...
      <div>
        <p>{{ item.creator_nickname || item.creator_username }}</p>
...
export default Vue.extend({
  name: 'weibo--list-comments',
  props: {
    articleId: { required: true, type: Number, default: 0 }
  },
  data() {
    return {
      comments: [] as any[]
  ...
  methods: {
    async init() {
      this.comments = await this.$Model.Weibo.postils(this.articleId);
    }
  }
});
</script>
